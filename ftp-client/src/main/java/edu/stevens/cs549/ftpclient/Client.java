...
public class Client {
	...
	public Client(String[] args) {
		try {
            ...
            IServerFactory factory = (IServerFactory) registry.lookup(serverName);
			...
			if (inputs.length == 2) {
				try {
					if (mode == Mode.PASSIVE) {
						 ...
						 * TODO
						 */
                        svr.put(inputs[1]); // tell the server to prepare to receive
                        try (Socket socket = new Socket(serverAddress, serverSocket.getPort());
                             BufferedOutputStream bos = new BufferedOutputStream(socket.getOutputStream());
                             BufferedInputStream bis = new BufferedInputStream(new FileInputStream(inputs[1]))) {
                            byte[] buf = new byte[8192];
                            int n;
                            while ((n = bis.read(buf)) != -1) {
                                bos.write(buf, 0, n);
						 ...
						 * TODO
						 */
                        final String fname = inputs[1];
                        new Thread(() -> {
                            try (Socket s = dataChan.accept();
                        ...
                        svr.put(inputs[1]); // trigger server to initiate the active data connection
					} else {
						msgln("GET: No mode set--use port or pasv command.");
					}
				} catch (Exception e) {
